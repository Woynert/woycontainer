...
#define WMAPSTR__TYPE Age
#define WMAPSTR__NAMESPACE name2age_map
#include "wmapstr.h"



void map_print(name2age_map *m) {
    printfd("\nPrinting map. First %d Last %d", ID_get(m->table.first_node), ID_get(m->table.last_node));
    int count = 0;
    name2age_map_It it = name2age_map_make_it(m);
...
TEST test_basic(void) {
    ...
    strview_t name_marco = cstr_SL("Marco");
    printfd("->>["PRIstrw"]<<--", PRIstrarg(name_marco));
    Age age = { 20 };
    ...
    strview_t name_polo = cstr_SL("Polo");
    age = (Age) { 1025 };
    ...
    strview_t name_available = cstr_SL("available Estimation of how much memory is available for starting new applications, without swapping. Unlike the data provided by the cache or free fields, this field takes into account page cache and also that not all reclaimable memory slabs will be reclaimed due to items being in use (MemAvailable in /proc/meminfo, available on kernels 3.14, emulated on kernels 2.6.27+, other‐ wise the same as free)");
    age = (Age) { 990099 };
    ...
    strview_t name_numbers = cstr_SL("one two three four five");
    age = (Age) { 12345 };
    ...
    int pair_count;
    pair_count = name2age_map_pair_count(m);
    name2age_map_remove(m, name_available);
    map_print(m);
    ASSERT_INT(pair_count-1, name2age_map_pair_count(m));
    pair_count = name2age_map_pair_count(m);
    name2age_map_remove(m, name_marco);
    map_print(m);
    ASSERT_INT(pair_count-1, name2age_map_pair_count(m));
    pair_count = name2age_map_pair_count(m);
    name2age_map_remove(m, name_numbers);
    map_print(m);
    ASSERT_INT(pair_count-1, name2age_map_pair_count(m));
    pair_count = name2age_map_pair_count(m);
    name2age_map_remove(m, name_polo);
    map_print(m);
    ASSERT_INT(pair_count-1, name2age_map_pair_count(m));
